...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.rise-cli${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.rise-cli${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.rise-cli', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.rise-cli', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.rise-cli', 'skills')
