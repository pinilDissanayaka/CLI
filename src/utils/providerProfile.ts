...
export const PROFILE_FILE_NAME = '.rise-cli-profile.json'
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  return buildLaunchEnv({
    ...
    goal:
      options?.goal ??
      normalizeRecommendationGoal(processEnv.RISE_CLI_PROFILE_GOAL),
...
export async function applySavedProfileToCurrentSession(options: {
  ...
  const nextEnv = await buildLaunchEnv({
    ...
    goal: normalizeRecommendationGoal(processEnv.RISE_CLI_PROFILE_GOAL),
    processEnv: baseEnv,
    getOllamaChatBaseUrl,
