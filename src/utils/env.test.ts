...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), 'rise-cli-env-test-'))
...
test('getGlobalClaudeFile: new install returns .rise-cli.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.rise-cli.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .rise-cli.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.rise-cli.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.rise-cli.json'))
})
