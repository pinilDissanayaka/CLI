...
test('CLI identity prefixes describe Rise CLI instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('Rise CLI')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('Rise CLI')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes Rise CLI instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('Rise CLI')
  expect(prompt[0]).not.toContain('Claude Code')
  expect(prompt[0]).not.toContain("Anthropic's official CLI for Claude")
})

test('built-in agent prompts describe Rise CLI instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('Rise CLI')
  ...
  expect(generalPrompt).toContain('Rise CLI')
  ...
  expect(explorePrompt).toContain('Rise CLI')
  ...
  expect(guidePrompt).toContain('Rise CLI')
  expect(guidePrompt).toContain('You are the Rise CLI guide agent.')
  expect(guidePrompt).toContain('**Rise CLI** (the CLI tool)')
