...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'rise-cli-wiki-ingest-'))
  tempDirs.push(dir)
  return dir
}
...
test('ingestLocalWikiSource creates a source note and updates log/index', async () => {
  ...
  expect(result.sourceNote.startsWith('.rise-cli/wiki/sources/')).toBe(true)
  ...
  expect(index).toContain(result.sourceNote.replace('.rise-cli/wiki/', './'))
})
