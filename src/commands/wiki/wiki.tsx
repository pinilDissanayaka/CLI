...
Manage the Rise CLI project wiki stored in .rise-cli/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized Rise CLI wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `Rise CLI wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }

  return [
    'Rise CLI wiki status',
    '',
    `Root: ${status.root}`,
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the Rise CLI wiki.`,
    '',
    `Title: ${result.title}`,
